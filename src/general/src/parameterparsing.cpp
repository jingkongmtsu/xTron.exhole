...
Parameters::Parameters(const string& input, const UInt& sec)
{
	ifstream inf;  //Input stream.
	//First, read the parameters.x file.
  string input_file = string(getenv("EMUL_SETTING_FILE")) 
                      + "/" + "parameters.x" ;
  inf.open(input_file.c_str(),ios::in);
  if (!inf) { cout << "ERROR: could not open parameters.x" << endl; abort(); }
	...
	//Now the parameters section of the input file.
	inf.open(input.c_str(),ios::in);
	if ( ! findSection(inf, "parameters", sec, true) ) return;
	...
	while(getline(inf,line)) {
		...
		if (l.getNPieces() != 2) {
			cout << line << endl;
			Excep excep("ParameterParsing","constructor",EXCEPTION_INPUT_PARAMETER_INVALID,
					"The %parameters section should be key/value pairs.");
			handleExcep(excep);
		}
		string keyword = l.findValue(0);
		string value   = l.findValue(1);
		w.capitalize(keyword);
		parameters.at(keyword) = value;
	}
	// close the input file
	inf.close();
}
		...
		const string& className, const UInt& sec, bool isCluster)
{
	...
	if ( ! findSection(inf, className,  sec, isCluster) ) return;
...
bool ParameterParsing::findSection(ifstream& inf, const string& className, 
	                                 const UInt& sec, bool isCluster)
{
	string line;
	WordConvert w;
