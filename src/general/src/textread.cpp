...
UInt textread::wordToUInt(const string& s) {
	UInt i;
	if ( ! WordConvert().toUInt(s,i) ) { cout << "wordToUInt fails for " << s << endl; abort(); }
	return i;
}

Int textread::wordToInt(const string& s) {
	Int i;
	if ( ! WordConvert().toInt(s,i) ) { cout << "wordToUInt fails for " << s << endl; abort(); }
	return i;
}

Double textread::wordToDouble(const string& s) {
	Double d;
	if ( ! WordConvert().toDouble(s,d) ) { cout << "wordToUInt fails for " << s << endl; abort(); }
	return d;
}

bool textread::wordCmpr(string s1, string s2) {
	return WordConvert().compare(s1, s2);
}
