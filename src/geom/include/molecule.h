...
namespace molecule {
	...
	class Molecule {
		...
		protected:
			...
			//!!Reform. tmp change for dump hole data.  Will be part of globalinfor later.
			string infile;           
		...
		public:
			 ...
			 */
			Molecule(const UInt& atomic, const string& input, UInt whichGeom);
			...
			const string& inputfile() const { return infile; }

			string getParam(const string& key) const { return params.getValue(key); }
	};
