...
///////////////////////////////////////////////////
Molecule::Molecule(const string& input, UInt whichGeom):unrestricted(false),
	        unitInAngstrom(true),section(whichGeom),multiplicity(1),
	        nalpha(0),nbeta(0),charge(0),params(input,whichGeom), infile(input)
{
	// open the input file
	ifstream inf;
	const char * input_file = input.c_str();
...
Molecule::Molecule(const UInt& atomic,const string& input, UInt whichGeom)
	:unrestricted(false),unitInAngstrom(true), section(whichGeom),
	multiplicity(1),nalpha(0),nbeta(0),charge(0),params(input,whichGeom),
	infile(input)
{
