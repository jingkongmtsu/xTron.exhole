...
// external functions
extern void doSP(const string& inf, const GlobalInfor&);
extern void fracSpin(const string& inf, const GlobalInfor&);
extern void frac1Job(const string& inf, const GlobalInfor&);

typedef void (*XMainPtr)(const string& inf, const GlobalInfor&);
...
int main(int argc, char* argv[])
{
	...
	GlobalInfor globInfo(inf);
	globInfo.print();

	//!!The following should be part of GlobalInfor.
	map<string,XMainPtr> xMains;
	xMains.insert(std::pair<string,XMainPtr>("FRAC1JOB",&frac1Job));
	xMains.insert(std::pair<string,XMainPtr>("FRAC3JOBS",&fracSpin));
	xMains.insert(std::pair<string,XMainPtr>("SPJOBS",&doSP));
	(*(xMains[globInfo.getXMain()]))(inf,globInfo);
	cout << "DOOOOONE" << endl; exit(0);
#if 0
