...
#if 0
    /* Conrad Lewis */
    /* Generating Mulliken Population Analysis */

    double sum_of_charges = 0.0;
    for (UInt is = 0; is <= den.getNSpin(); ++is)
    {

        double tot_alpha_spin = 0.0;
        double tot_beta_spin = 0.0;
        if(is == 0)
        {
            std::cout << "\nMulliken Charge Analysis:" << std::endl;
            std::cout << "\nAlpha: " << std::endl;
        }
        else if(is == 1)
        {
        ...
        // Getting density matrix of spin 'is'
        const Mtrx D = den.getMtrx(is);
        Mtrx P(D.getRow(), D.getCol());
        UInt nRows = D.getRow();

        // Obtaining overlap matrix
        UInt nRow = -1, nCol = -1;
        oneEMtrx.getDim(TWO_BODY_OVERLAP,nRow,nCol); 
        Mtrx S(nRow,nCol);
        oneEMtrx.getM(TWO_BODY_OVERLAP,S);

        // Element wise multiplication of overlap and density matrices
        P.element_wise_mult(D,S);
    
        // Obtaining the diagonal and off diagonal contributions to each orbital 
        // from the P matrix.
        double gross_pop[nRows+1];
        for(UInt i = 0; i < nRows; ++i)
        {
            ...
            gross_pop[i] = diag + off_diag;
        }

        signed int count = -1;
        UInt num_shells = ms.getNAtomShells();
        for(UInt i = 0; i < num_shells; ++i)
        {
            double orbital_pop = 0;
            UInt low  = 0;
            UInt high = 0;

            UInt atom_charge = mol.getAtomic(i);         
            const AtomShell& atom_shell_1 = ms.getAtomShell(i);
            const AtomShell& atom_shell_2 = ms.getAtomShell(i+1);
            UInt shell_1 = atom_shell_1.getBasisStartIndex(TYPE_NORM);
            UInt shell_2 = atom_shell_2.getBasisStartIndex(TYPE_NORM);

            // Using the high and low index values to sum the orbital
            // contributions for each element appropriately.
            if(i == num_shells - 1)
            {
                shell_2 = nRows;
            }
            ...
            for(UInt j = 0; j <= nRows; ++j)
            {
                if(j >= low and j < high)
                {
                    orbital_pop += gross_pop[j];
                }
            }

            // Generating atomic symbol and printing out information to
            // the output file.
            // mol.getSymbol uses the atom index, NOT the atomic charge
            count             = count + 1;
            double tot_charge = atom_charge - orbital_pop;
            
            string symbol     = mol.getSymbol(i);

            if(is == 0)
            {
                printf("%-5s %-5d %-12.9f\n",symbol.c_str(),count,orbital_pop);
                sum_of_charges  += tot_charge;
                tot_alpha_spin += tot_charge;
            }
            else
            {
                printf("%-5s %-5d %-12.9f\n",symbol.c_str(),count,orbital_pop);
                sum_of_charges  -= tot_charge;
                tot_beta_spin += tot_charge;
            }
        }
    
        if(is == 0)
        {
            printf("Total Alpha: %-12.9f\n", tot_alpha_spin);
        }
        else if(is == 1)
        {
            printf("Total Beta: %-12.9f\n", tot_beta_spin);
        }

    }
    printf("\nSum of Mulliken Atomic Charges: %.9f",sum_of_charges);
    cout << "\n" << endl;
#endif
////////////////////////////////////////////////////////////////////////////////
