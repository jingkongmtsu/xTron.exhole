...
#define init_func_deriv_1      init_func_deriv_1_
...
#define kp14ec1p               kp14ec1p_
#define kp14exc                kp14exc_
#define functional_psts        functional_psts_
#define b05ndopp1p             b05ndopp1p_
#define b05ndpar1p             b05ndpar1p_
#define b05ndpar1pxx           b05ndpar1pxx_

// For hole functions.
#define br89xhole_s            br89xhole_s_
#define brrelaxhole_s          brrelaxhole_s_
#define hirao00xhole_s         hirao00xhole_s_

// For other functions.
#define becke05_f              becke05_f_
...
extern "C" {
	...
	void init_func_deriv_1_(const Int* VAR_INFOR, Int* D1VARS);
	...
	void br94coor_op(const Int* INFOR, const Int* NDEN,const Int* NA,const Int* NB,const Int* NG,
			const double* THRESH, const double* RhoA,const double* RhoB, const double* DRA,
			const double* DRB, const double* TauA,const double* TauB,const double* LapA,
			const double* LapB, double* F,double* D1F);

	void br94corropp1p(const Int* NDEN, const Int* NA, const Int* NB, const double* THRESH, 
	                const double* RA, const double* RB, 
	                const double* GAA, const double* GBB,
	                const double* TA, const double* TB,
	                const double* LA, const double* LB,
	                double* F0, double* DRhoA, double* DGAA, double* DTA,
	                double* DLA,
	                double* DRhoB, double* DGBB, double* DTB, 
	                double* DLB);
	void br94corroppfzab(double* FZ, double* FZ_UA, double* FZ_UB, const Int* NDEN, 
	                     const double* THRESH, const double* UA, const double* UB);
  ...
  void becke05_ndpar1(const Int* INFOR,const Int* NDEN,const Int* NG, const Int* NA,
      const Int* NB, const double* THRESH, const double* c_ndpar_cap,
      const double* VAL_P, const double* VAL_Q, const double* HIRSHWTS,
      const double* RhoA, const double* RhoB, const double* DRA, const double* DRB,
      const double* TauA, const double* TauB, const double* LapA, const double* LapB,
      const double* EXA, const double* EXB, double* F, double* D1F);
   ...
   void kp14exc(const Int* iFunc, double* F, double* D1F, const double* D1FEXCH, const double* W, 
	...
	void kp14ec1(const Int* NDPARMETHOD, const Int* INFOR, const Int* NDEN, const Int* NG, const Int* NA, const Int* NB, const double* b,
			const double* alpha,const double* c_ndpar, const double* c_ndpar_cap, const double* THRESH, 
			const double* VAL_P, const double* VAL_Q, const double* HIRSHWTS,
			const double* RhoA, const double* RhoB,
			const double* DRA, const double* DRB, const double* TauA, const double* TauB,
	...
	void functional_psts(const Int* INFOR,const Int* NG,const Int* NDEN,const double* THRESH, 
			...
			double* F, double* D1F);
	void b05ndopp1p(const double* THRESH, const double* VAL_P,
	                const double* RA, const double* RB, 
	                const double* GAA, const double* GBB,
	                const double* TA, const double* TB,
	                const double* LA, const double* LB,
	                const double* UA, const double* UB,
	                const double* HIRWT,
	                double* F0, double* DRhoA, double* DGAA, double* DTA,
	                double* DLA, double* DUA,
	                double* DRhoB, double* DGBB, double* DTB, 
	                double* DLB, double* DUB);

	void b05ndpar1p(const int* NA, const int* NB, 
	                const double* THRESH, const double* VAL_P, const double* VAL_Q,
	                const double* RA, const double* RB, 
	                const double* GAA, const double* GBB,
	                const double* TA, const double* TB,
	                const double* LA, const double* LB,
	...
	void kp14ec1p(const int*, const int*, const int*, const int*,
	...
	void b05ndpar1pxx(const int* NA, const int* NB, 
	                const double* THRESH, const double* VAL_P, const double* VAL_Q,
	                const double* RA, const double* RB, 
	                const double* GAA, const double* GBB,
	                const double* TA, const double* TB,
	...
	void br89xhole_s(double* hx, const double* sVal, const double* RhoA, 
	    ...
	    const Int* NDEN);
	void brrelaxhole_s(double* hx, const double* sVal, const double* thresh, 
	    const double* RhoA, 
	    const double* RhoB, const double* DRA, const double* DRB, const double* LapA, 
	    const double* LapB, const double* TauA, const double* TauB, 
	    const double* EXA, const double* EXB, const Int* Ng, 
	    const Int* NDEN);
	...
	void becke05_f(const double* THRESH, const double* p, const double* hirWts,
	               const double* RA, const double* GAA, const double* TA, 
	               const double* LA, const double* EXRA,
	               const double* RB, const double* GBB, const double* TB, 
	               const double* LB, const double* EXRB,
	               double* F0, double* DRhoA, double* DGAA, double* DTA, double* DLA, double* DUA,
	               double* DRhoB, double* DGBB, double* DTB, double* DLB, double* DUB);
	/*
