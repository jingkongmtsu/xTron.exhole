...
c       write(6,*)"N1U: ", D1U
      ...
      SUBROUTINE brRelaxHoleAB(a,b,n1,THRESH,Rho,Gam,Tau,Lap,UX)
c
c    ******************************************************************
c    *                                                                *
c    *  evaluates the N_{eff} for Becke05 exchange functional         *
...
C
       ...
       DQDT = ZERO
       DQDL = ZERO
       HIRWT = 0D0  !!! For non-b13 holes only for now. -jk
       CALL BR89_CURVATURE(THRESH,Rho,Gam,Tau,Lap,HIRWT,Q,
     $ DQDR,DQDG,DQDT,DQDL)
c       write(6,*)"Q: ", Q
c       write(6,*)"DQDR: ", DQDR
...
c       write(6,*)"DXDL: ", DXDL
       ...
       DFDL = ZERO
       DFDU = ZERO
       a = 0D0   ! zero case will lead to 0 hole value in brhole_s
       b = 0D0
       IF (DABS(Q) >THRESH .and. DABS((X-TWO)/(X*Q))>THRESH) THEN
          QX    = Q*X
          QX2   = QX*QX
...
c          write(*,*) 'In becke05_neff: x, a, b, N1', x, a, b, N1
       ENDIF
       RETURN 
       END
...
c         write(6,*)"beta neff: ", N2
     ...
     $   RB,GBB,TB,LB,UB,A2)
         ...
         ! Change here: Uncomment the two lines below for atomic charges.
c         OD1 = RA
c         OD2 = RB
         F(i) = OD1 + OD2
      END DO
...
c         write(6,*)"our UB: ",UB 
         ...
         call b05ndopp1p(THRESH,VAL_P,
     $     RA,RB,GAA,GBB,TA,TB,LA,LB,UA,UB,HIRWT,
     $     F0,DRhoA,DGAA,DTA,DLA,DUA,DRhoB,DGBB,DTB,DLB,DUB,DGAB)
...
C         write(6,*)"for grid: ", i
     ...
     &+ DRB(i,3)*DRB(i,3)
         ...
         call b05NDPar1P(NA, NB, THRESH, VAL_P,VAL_Q,
     $     RA,RB,GAA,GBB,TA,TB,LA,LB,UA,UB,HIRWT,
