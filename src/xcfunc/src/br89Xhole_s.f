...
c     REAL*8 hx(NGrid,*),RA(NGrid),RB(NGrid),D1RA(NGrid,3),
     ...
     $   RLAPA(NGrid),RLAPB(NGrid),TA(NGrid),TB(NGrid),D1RB(NGrid,3)
      ...
      REAL*8 pi,pi13,pi23,ba,bb,aa,ab,n,sVal,ee1,ee2,ee3,ee4,h2a,h2b
...
c
           ...
           IF (RA(i).gt.Tol) then
              ...
              CALL brbrxb_hx(ba,aa,Norm,D1,gra,DLAPA,TAUA)
              write(*,*) "In br89Xhole_s. D1, ba, aa, Norm", 
     $                   D1, ba, aa, Norm
c
             call brhole_s(ba,aa,Norm,sVal,hx(i))
           ENDIF   !  RA,RB vs Tol
c
      ENDDO  !  NGRID
        RETURN
      ...
      subroutine brhole_s(ba,aa,n,sVal,hx)
      implicit none
      real*8 ba,aa,sVal,hx,n
      !Internal
      real*8 ee1,ee2,ddem,pi,h1a,h2a,Tol2
      parameter(pi=3.1415926535897930d0)
      Tol2 = 1.0D-07
...
c           IF(DABS(ddem).gt.Tol) then
      ...
      SUBROUTINE brrelaxhole_s(hx,sVal,THRESH,RA,RB,D1RA,D1RB,
     $ RLAPA,RLAPB,TA,TB,EXA,EXB,NGrid,NDEN)
c
c    ******************************************************************
c    *                                                                *
c    *  reference: A. D. Becke and E. R. Johnson, J. Chem. Phys. 122, *
c    *             154104 (2005).                                     *
c    * Evaluating the BR89 hx on a grid. Alpha only!!                 *
c    ******************************************************************
c
      IMPLICIT NONE 
#include "fderiv1.inc"
      REAL*8 hx(NGrid),RA(NGrid),RB(NGrid),D1RA(NGrid,3),
     $   RLAPA(NGrid),RLAPB(NGrid),TA(NGrid),TB(NGrid),EXA(NGrid),
     $   EXB(NGrid),D1RB(NGrid,3),THRESH
      REAL*8 D1,D2,DLAPA,DLAPB,TAUA,TAUB,UX,
     $       gra,grb,Tol,h1a,h1b,ddem,Tol2
      REAL*8 pi,pi13,pi23,ba,bb,aa,ab,n,sVal,ee1,ee2,ee3,ee4,h2a,h2b
c
      INTEGER NGrid
      INTEGER i
...
c
      DO i = 1,NGrid
        ...
        IF (RA(i).gt.Tol) then
              ...
              DLAPA = RLAPA(i)
              UX = EXA(i)
           CALL brRelaxHoleAB(aa,ba,n,THRESH,D1,gra,TAUA,DLAPA,UX)
           write(*,*) "In brrelaxhole_s. D1, ba, aa, n", D1, ba, aa, n
           call brhole_s(ba,aa,n,sVal,hx(i))
        ENDIF   !  RA,RB vs Tol
c
      ENDDO  !  NGRID
      RETURN
      END
c
       SUBROUTINE brbrxb_hx(b,aaa,Norm,DN,GR,DLAP,TAU)
c
...
c  function of rho, and Qs:
           IF(dabs(QS).gt.dtol) then
               yy = twthr*pi23*DN53/QS*Norm**third
           else
               yy=dtol2
           endif
...
c analytic part ends here
           b = 0.50d0*x_y*XEXP/pi13/DN13*Norm**third
