...
C
      SUBROUTINE KP14EC1p(
     $             INFOR,NDEN,NA,NB,b,alpha,c_ndpar,THRESH,VAL_P,VAL_Q,
     $             RA,RB,GAA,GBB,TA,TB,LA,LB,UA,UB,HIRWT,
     $             TMP_DRA,TMP_DRB,uspar,
     $             uspar_RA, uspar_GAA, uspar_TA, uspar_LA, uspar_UA,
     $             uspar_RB, uspar_GBB, uspar_TB, uspar_LB, uspar_UB,
     $             F,TF1,TF2,
     $             DRhoA,DGAA,DTA,DLA,DUA,DRhoB,DGBB,DTB,DLB,DUB)
c
c
c    ******************************************************************
c    *                                                                *
...
c    *                                                                *
c    *   fenglai liu, E. Proynov, J. Kong                             *
c    *                                                                *
c    ******************************************************************
c
      IMPLICIT NONE
      ...
      INTEGER NA,NB
      INTEGER INFOR(*)
      REAL*8 b,alpha,c_ndpar,PI
      REAL*8 C
      REAL*8 THRESH
      REAL*8 HIRWT
      REAL*8 VAL_P,VAL_Q
      !Return variables.
      REAL*8 F,TF1,TF2,DRhoA,DGAA,DTA,DLA,DUA,DRhoB,DGBB,DTB,DLB,DUB
...
C
      ...
      CALL INIT_FUNC_DERIV_1(INFOR,D1VARS)
         ...
         ! TMP_DRA, TMP_DRB, uspar and TMP_D1F are now passed in.
...
c         write(6,*)"qac deriv on EXB: ",QAC_Z*z_UB
         ...
         DRhoA = QAC_Z*z_RA*usm  + QAC*usm_RA
         DGAA  = QAC_Z*z_GAA*usm + QAC*usm_GAA
         DTA   = QAC_Z*z_TA*usm  + QAC*usm_TA
         DLA   = QAC_Z*z_LA*usm  + QAC*usm_LA
         DUA   = QAC_Z*z_UA*usm  + QAC*usm_UA
         DRhoB = QAC_Z*z_RB*usm  + QAC*usm_RB
         DGBB  = QAC_Z*z_GBB*usm + QAC*usm_GBB
         DTB   = QAC_Z*z_TB*usm  + QAC*usm_TB
         DLB   = QAC_Z*z_LB*usm  + QAC*usm_LB
         DUB   = QAC_Z*z_UB*usm  + QAC*usm_UB
