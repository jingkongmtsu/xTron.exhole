...
void AtomGrids::preCheck() const
{
	...
	if (nAngPts == 1)
	{
		cout << "WARN: nAngPts == 1" << endl;
	}
	else if (! usePruneGrids()) {
...
void AtomGrids::formQuadraturePtsWts(const XCIntJobInfor& infor)
{
	...
	if ( nRadPts == 1 && nAngPts == 1 )
	{
		cout << "WARN: Doing 1 grid point read from input.\n";
		radialPts[0] = wordToDouble(infor.param("1point"));
		radialWts[0] = 1E0;
	}
	else
	{
	...
	for(UInt i=0; i<nRegions; i++) {
		...
		else if ( nAng == 1)
		{
			tmpAngPts[0] = 0E0;
			tmpAngPts[nAng] = 0E0;
			tmpAngPts[2*nAng] = 1E0;
			tmpAngWts[0] = 1E0;
		}
		else
		{
			cout << "ERROR: Wrong nAng < 1\n"; abort();
		}
