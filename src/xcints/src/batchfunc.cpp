		...
		const XCVar& xcvar, XCEnergyInfor& xcEList) 
{
	...
	//??jk. This vector is for the data from each functional.  nTempFunc 
	//instead of '1' is used for xc energy profiling.
	...
	XCFuncCalc xcfuncCalc(infor, ng, nDen, nAlpha, nBeta, bvar, tol, xcvar, 
	                      xcfunc, hirWeights);
	...
	if (nEXFunc > 0) {
		...
		for(UInt i=0; i<xcfunc.getLenEx(); i++) {
			...
			//??jk.  This shows that the accumulation inside the functional
			...
			if (exname == "SLATER") {
				...
				becke05_ndop(infor,&nDen_,&ng_,&tol_,&p,hirw,rhoA,rhoB,DRA,DRB,TA,TB,LA,LB,
					EXRA,EXRB,TF,TD1F);
			}else if (exname == "M_B05_NDOP") {
				xcfuncCalc.calcB05NDOpp(TF,TD1F,nTempFunc,nD1FWithGamma);
			...
			}else if (exname == "M_B05_NDPAR") {
				xcfuncCalc.calcB05NDPar(TF,TD1F,nTempFunc,nD1FWithGamma);
			}else if (exname == "M_BR94CORR_OPP_X3") {
				xcfuncCalc.calcBR94CorrOppX3(TF,TD1F,nTempFunc,nD1FWithGamma);
			}else{
	...
	if (nECFunc > 0) {
		...
		for(UInt i=0; i<xcfunc.getLenEc(); i++) {
			...
			if (ecname == "PW92C") {
				...
				Int NA = static_cast<Int>(nAlpha);
				Int NB = static_cast<Int>(nBeta);
				br94coor_par(infor,&nDen_,&NA,&NB,&ng_,&tol_,rhoA,rhoB,DRA,DRB,TA,TB,LA,LB,TF,TD1F);
			}else if (ecname == "M_BR94COOR_OP") {
				xcfuncCalc.calcBR94CorrOpp(TF,TD1F,nTempFunc,nD1FWithGamma);
			}else if (ecname == "M_BR94CORR_OPP_X0") {
				xcfuncCalc.calcBR94CorrOppX0(TF,TD1F,nTempFunc,nD1FWithGamma);
			}else if (ecname == "M_BR94CORR_OPP_X1") {
				xcfuncCalc.calcBR94CorrOppX1(TF,TD1F,nTempFunc,nD1FWithGamma);
			}else if (ecname == "M_BR94CORR_OPP_X2") {
				xcfuncCalc.calcBR94CorrOppX2(TF,TD1F,nTempFunc,nD1FWithGamma);
			}else if (ecname == "BR94COOR_OP") {
				Int NA = static_cast<Int>(nAlpha);
				Int NB = static_cast<Int>(nBeta);
				br94coor_op(infor,&nDen_,&NA,&NB,&ng_,&tol_,rhoA,rhoB,DRA,DRB,TA,TB,LA,LB,TF,TD1F);
			...
			}else if (ecname == "M_KP14C") {
				DoublePrecVec TF1_array(2*ng);
				double* TF1   = &TF1_array.front();
				xcfuncCalc.calcKP14C(TF,TF1,TD1F,nTempFunc,2,nD1FWithGamma);
				if (xcEList.doXCEnergyProfiling()) {
					xcEList.updateEComponent(TF1,ecname,1);
					xcEList.updateEComponent(&TF1[ng],ecname,2);
				}
