...
{
	Int iAtom = wordToInt(infor.param("1atom"));
	if (iAtom >= 0)   //1 atom case.  designed for 1 point calc.
	{
		cout << "iAtom " << iAtom << endl; 
		MolShell ams(ms.getAtomShell(UInt(iAtom)));
		MolShellSize amss(ams,infor.getThresh());
		amss.print();
		init(ams, amss, infor);
	}
	else
	{
		init(ms, shellSize, infor);  //normal case.
	}
}


void MoleGrids::init(const MolShell& ms, const MolShellSize& shellSize, const XCIntJobInfor& infor)
{
