		...
		bool printTiming, bool printMatrix)
{
  ...
  if ( wordCmpr(infor.param("XCOper"),"true") )
  {
		XCInput xcInp(xcfunc,ms,denMtrx);
		XCIterInput xcItInp(spData,molGrids,size,xcvar,infor,cartDen,atomDenMtrx);
		XCProp* xcProp = makeXCProp(xcItInp,infor.param("XCProp"), denMtrx.getNSpin(), 
			              wordToUInt(infor.param("nSValues")));
		XCIntsOper xcOper(xcInp,xcItInp,*xcProp); 
		UInt len = molGrids.getNTotalBatches();
		parallel_for(blocked_range<UInt>(0,len), xcOper);
		xcProp->dump();
		delete xcProp;
		cout << "HOLE IS DONE" << endl;
		abort();
  }
...
XCProp* XCInts::makeXCProp(const XCIterInput& xcItInp, const string& xcProp, 
	                         UInt nSpin, UInt nSValue)
{
	if ( wordCmpr(xcProp, "HFXHole" ) ) 
	{ return new HFXHole(nSpin,nSValue,infor.param("holefile"),xcItInp); }
	else if ( wordCmpr(xcProp, "MetaGGAHole") ) 
	{ return new MetaGGAHole(nSpin,nSValue,infor.param("holefile"),xcItInp); }
	else if ( wordCmpr(xcProp, "HyperGGAHole") ) 
	{ return new HyperGGAHole(nSpin,nSValue,infor.param("holefile"),xcItInp); }
	else { cout << __FILE__ << ", line " << __LINE__ << "  Wrong xcProp."; abort(); }
}
