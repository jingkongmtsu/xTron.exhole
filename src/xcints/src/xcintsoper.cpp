...
void XCIntsOper::operator()(const blocked_range<UInt>& r) const
{
	...
	for( UInt iBatch=r.begin(); iBatch!=r.end(); ++iBatch ) {
		...
		//Reform. Hirshfeld weights should go into batchvar, and be asked for in 
		//xcfunc.conf like other variables. The only difference is that it does not 
		//depend on density. 
		...
		//Reform.  The following two blocks are temporary for studying functionals
		//in conjuction with hole calculations.  In the future, functionals seem 
		//to belong to integrals such as xcenergy or xcmatrix more logically.
		// if we do XC energy profiling,
		// we need to initialize it before pass
		// it to batch func
		XCEnergyInfor xcEProfile0(xcItInp.infor,xcInp.xcfunc);
		if (xcEProfile0.doXCEnergyProfiling()) {
		  xcEProfile0.init(bg);
		}
		
		// batch functional
		BatchFunc bFunc(bVar,xcInp.denMtrx,xcInp.xcfunc,xcItInp.xcvar,xcItInp.infor);
		if (xcInp.xcfunc.useHirshfeldWeights()) {
		  bFunc.formBR89EXHoleWeights(xcInp.ms,sigList,hirshfeld);
		}
		bFunc.doFuncDeriv(bVar,xcInp.xcfunc,xcItInp.xcvar,xcEProfile0);
		if (xcItInp.infor.doDebug("batchfunc")) {
		  bFunc.print();
		}
